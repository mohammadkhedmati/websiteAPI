...
def DomainAge(request):
    
    domain = request.split("//")[-1].split("/")[0].split('?')[0]
    w = whois.whois(domain)
    return w.creation_date

def AlexaRank(request):
...
def  IndexedPage(request):
    ...
    indexed_pages=count_indexed_pages("http://flyandlure.org")
